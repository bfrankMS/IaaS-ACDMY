<#
    ...
    ***************************************
        ...
        Create VNET, Subnets, NICS, NSGs, IPs, AVSet...
        ...and link it all together for a VM...
        ...called VMOne!

    ***************************************
    This script will create the a customized Azure VMs 
...
$AVSet = New-AzAvailabilitySet -ResourceGroupName $RG -Name $AVSetName -Location $Location -PlatformUpdateDomainCount 5 -PlatformFaultDomainCount 2 -Sku Aligned
...
$Publisher = (Get-AzVMImagePublisher -Location $location |  Out-GridView -PassThru -Title 'Select publisher e.g. MicrosoftWindowsServer'   ).PublisherName 
...
$VMImageSKU = (Get-AzVMImageSku -Location $Location -PublisherName $PublisherOffer.PublisherName -Offer $PublisherOffer.Offer).Skus | Out-GridView -PassThru -Title 'Select SKU e.g. 2019-Datacenter'
...
$VMImage = Get-AzVMImage -Location $Location -PublisherName $PublisherOffer.PublisherName -Offer $PublisherOffer.Offer -Skus $VMImageSKU | Sort-Object version -Descending | Out-GridView -PassThru -Title 'Select Image version'
...
$VM = Set-AzVMOperatingSystem -VM $VM -ComputerName $VMName -Credential $Credential -ProvisionVMAgent -EnableAutoUpdate -Windows
...
#New VM
$job = New-AzVM -ResourceGroupName $RG -Location $location -VM $VM -Verbose -AsJob  #runs in background

#do something here while the vm spins up in azure...e.g. another VM

#when you have to wait you can do e.g.
Wait-Job -Job $job 
